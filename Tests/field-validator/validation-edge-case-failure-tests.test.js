...
describe("Edge Case Failure Tests", () => {
  ...
  test("should handle fieldId mismatch gracefully", async () => {
    ...
    expect(
      validationResults.details?.nonExistentField?.[0]?.message ?? ""
  ...
  test("should handle empty string values in REGEX validation", async () => {
    ...
    expect(validationResults.hasErrors).toBe(true);
    expect(validationResults.details?.email?.[0]?.message ?? "").toBe(
  ...
  test("should handle rules with unsupported types gracefully", async () => {
    const formData = {
    ...
    expect(validationResults.details?.username?.[0]?.message ?? "").toBe(
      "Unsupported rule type: UNSUPPORTED_RULE"
    );
  });

  test("should handle unexpected data types in fields gracefully", async () => {
    const formData = {
    ...
    expect(validationResults.details?.age?.[1]?.message ?? "").toBe(
  ...
  test("should handle form data with unexpected structures gracefully", async () => {
    const formData = {
    ...
    // Expect overall validation to have errors
    expect(validationResults.hasErrors).toBe(true);

    // Expect a system-level error for invalid form data
    expect(validationResults.details?.SYSTEM?.[0]?.message ?? "").toBe(
      "Invalid form data provided"
    );

    // Check that the error is tagged as a system-level issue
    expect(validationResults.details?.SYSTEM?.[0]?.fieldId ?? "").toBe(
      ""
    );
  });

  test("should handle multiple rules on the same field with conflicting results gracefully", async () => {
    const formData = {
    ...
    expect(validationResults.hasErrors).toBe(true);
    const errorMessages =
      validationResults.summary?.map((error) => error.message) ?? [];

    expect(errorMessages).toEqual(
      expect.arrayContaining(["Age must be less than 16."])
    );
  });
});
